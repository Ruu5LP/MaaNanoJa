...
## Phase 14: トップページの入口と保存範囲の明確化

### Overview

トップページで「ゲストで試す」「Googleでログインする」「共有ルームを作る・参加する」の役割を明確に分ける。ゲストは現在のタブに一時保存するお試し利用、Googleログイン後の共有ルームは永続保存・複数人利用という境界を、開始前と利用中の両方で説明する。

### Architecture Decisions

- **ゲストと共有ルームを別物として扱う:** ゲスト開始では`POST /api/rooms`を呼ばず、既存の`sessionStorage`だけを利用する。「ゲストとしてルームを作成」のように、共有ルームと誤解される表現は使わない。
- **共有ルームの新規作成はログイン後の操作とする:** 公開環境ではGoogleログインを有効にし、未ログイン時はログイン導線へ誘導する。認証無効環境の後方互換挙動は、別途仕様変更するまで維持する。
- **入口を保存先で説明する:** ゲストは「試す」、Googleログインは「保存・管理する」、共有ルームは「みんなで同じデータを使う」という説明に統一する。
- **状態を常に可視化する:** ゲスト開始前、ゲストデータが残っている状態、ゲスト利用中、ログイン済み、ログイン失敗をそれぞれ区別して表示する。

### Task List

### Task 41: トップページの入口構成とコピーを整理 [x]

- [x] ゲスト利用とGoogleログインを同じ階層・同程度の視認性で表示する
- [x] ゲストの保存範囲、タブを閉じたときの挙動、ログイン後に共有ルームへ保存できることを説明する
- [x] 「みんなで使う」を、ルーム作成・招待コード参加・対局データ共有の流れとして説明する
- [x] ページの主見出しを`h1`にし、トップページの見出し階層を整える

**Verification:** 初回表示と390px幅で、3つの入口の違いと次の行動が一読で分かることを確認する

**Dependencies:** Task 40

**Files likely touched:** `src/views/LandingView.tsx`, `src/views/RoomEntry.tsx`, `src/styles.css`

**Estimated scope:** Medium

### Task 42: ゲスト状態と保存先の表示を統一 [x]

- [x] ゲスト開始前は「ゲストとして始める」、既存データがある場合は「このタブの続き」と表示し、単独の「前回の続き」で誤解させない
- [x] ゲスト利用中は「ゲストモード」「一時保存中」「タブを閉じると消える」を常に表示する
- [x] ゲスト利用中のGoogleログイン導線を「このデータを共有ルームへ保存する」という目的付きで表示する
- [x] 設定画面・トップページ・ゲスト用ヘッダーで、保存範囲の説明を同じ意味にそろえる

**Verification:** ゲスト開始、ページ更新、ゲストデータがある状態、Googleログインへの遷移、保存成功後の共有ルーム遷移を手動確認する

**Dependencies:** Task 41

**Files likely touched:** `src/App.tsx`, `src/views/RoomEntry.tsx`, `src/views/SettingsView.tsx`, `src/styles.css`

**Estimated scope:** Medium

### Task 43: 共有ルーム作成のログイン導線と状態を明確化 [x]

- [x] 未ログイン時の「新しいルームを作る」は、ログイン必須であることをボタン付近に明示する
- [x] アカウント情報の取得中は共有ルーム作成ボタンを誤って押せない状態にする
- [x] アカウント情報の取得に失敗したとき、作成ボタンを有効に見せず、再試行またはログイン確認へ導く
- [x] ログイン済みの場合は、ルーム作成後に所有ルームとして扱われることを説明する
- [x] 公開環境の認証有効時に、Workerの作成制約とクライアントの表示が一致することを確認する

**Verification:** 未ログイン、ログイン済み、認証情報取得中、認証API失敗の4状態で、作成・参加ボタンの状態とエラー文言を確認する。Worker契約テストと`npm run check`を実行する

**Dependencies:** Task 41

**Files likely touched:** `src/App.tsx`, `src/views/RoomEntry.tsx`, `src/lib/account.ts`, `worker/index.ts`, `worker/contract.test.ts`

**Estimated scope:** Medium

### Task 44: 仕様書・利用説明・公開前チェックを更新 [x]

- [x] README、SETUP、ゲストモード仕様に、ゲスト・ログイン・共有ルームの境界を反映する
- [x] 「新しいルームを作るにはGoogleログインが必要」「招待コードは既存ルームへの参加用」と明記する
- [x] トップページの文言と仕様書の表現が一致することを確認する
- [x] `npm run check`、`npm run build`、390px幅とデスクトップ幅の手動確認を完了する

**Verification:** ドキュメントレビュー、主要導線の手動確認、全チェックコマンド

**Dependencies:** Task 42, Task 43

**Files likely touched:** `README.md`, `SETUP.md`, `docs/guest-mode-spec.md`, `tasks/plan.md`, `tasks/todo.md`

**Estimated scope:** Small

### Checkpoint: Landing Entry Clarity Complete

- [x] ゲストは一時保存、Googleログイン後は共有ルーム保存、招待コードは既存ルーム参加という違いが一目で分かる
- [x] ゲスト利用中であることと、データが消える条件が常に分かる
- [x] 未ログイン状態で共有ルーム作成を開始すると、Googleログインへ自然に誘導される
- [x] `npm run check` と `npm run build` が成功する
...
ルーム所有者が、対局履歴・進行中データ・メンバー紐付けを含む共有ルーム全体を明示的に削除できるようにする。削除は確認ダイアログを通し、Worker側でも所有者権限を検証する。

- [x] Task 45: 所有者限定のルーム削除APIを追加する
- [x] Task 46: 設定画面からルーム削除を実行できるようにする
- [x] Task 47: ルーム削除の仕様・契約テスト・全体検証を完了する

### Checkpoint: Room Deletion Complete

- [x] 所有者だけが削除でき、非所有者・未認証ユーザーは拒否される
