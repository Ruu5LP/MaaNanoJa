...
- `GET /api/rooms/:code/preview`: 認証済みユーザーに、ルーム作成者の表示名と自分の参加状態だけを返す。ルーム本体や対局データは返さない。
...
- Googleログイン有効環境で、未ログインのユーザーがルームURLを開いた場合は、ルーム内容を取得せず、元のURLへ戻るログイン導線を表示する。
- ログイン済みでも未参加のユーザーがルームURLまたはトップの招待コードから開いた場合は、作成者の表示名を表示し、確認操作の後にだけmemberとして登録する。
