...
ルームURLなしで開いた場合は、Googleログインなしで試せるお試しモード、またはログイン後の共有ルーム作成・参加を選べます。

## 使い方（アプリ内）

1. すぐ試す場合は「ログインなしで試す」を選ぶ。お試しデータはタブを閉じるまでの一時保存です。
2. 永続保存・ルーム共有を使う場合はGoogleログイン後、「新しいルームを作る」または共有されたルームURLから参加する。
...
共有ルームの変更はCloudflare APIを通って同じルームのD1へ保存されます。別端末では同じルームURLを開いてください。お試し中の変更はタブ内だけに保存されます。お試し中にGoogleログインすると、「クラウドに保存」からデータを新しい共有ルームへ移行できます。
...
ログイン前のお試しモードでは、対局の記録・成績・JSON書き出しを利用できます。お試しデータは`sessionStorage`に保存され、タブを閉じると破棄されます。クラウドへの永続保存、共有ルームの作成・参加、JSONから共有ルームを作る操作にはログインが必要です。詳しい境界は[お試しモード仕様](docs/guest-mode-spec.md)を参照してください。
