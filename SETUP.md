...
ルームURLなしで開いた場合は、ログインせずに使うか、ログイン後に共有ルームを作成・参加するかを選べます。

## 使い方（アプリ内）

1. すぐ使う場合は「このまま使う」を選ぶ。ログインせずに入力した内容は、タブを閉じるまでの一時保存です。
...
共有ルームの変更はCloudflare APIを通って同じルームのD1へ保存されます。別端末では同じルームURLを開いてください。ログインせずに入力した内容はタブ内だけに保存されます。入力後にGoogleログインすると、「このデータを保存する」から新しい共有ルームへ移行できます。
