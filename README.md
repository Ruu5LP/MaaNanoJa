...
いつも使っている [namimori 氏の麻雀集計スプレッドシート](https://hirokuasaku-live.blogspot.com/2022/08/mahjong-spreadsheet-new.html) のスコア計算を再現したうえで、**局ごとの記録**を足して統計を出せるようにしています。ログイン後の共有ルームはCloudflare D1へ保存します。未ログインのお試しモードはタブ単位の一時保存で、タブを閉じると消えます。
...
- **お試しモード**：Googleログインなしで記録・成績を試せます。データは`sessionStorage`に一時保存され、クラウドへの永続保存や共有にはログインが必要です。
...
- 未ログインのお試し状態は`sessionStorage`へ保存し、クラウド同期や`localStorage`への永続保存は行いません。
