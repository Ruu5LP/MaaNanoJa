...
- **Googleログイン**：Cloudflare AccessのGoogleログインを有効にすると、初回ログイン時にアカウントが自動作成され、作成したルームが「自分のルーム」に残ります。
...
- Google OAuthの認証情報やセッションはMaaNanoJaに保存せず、Cloudflare Accessへ委譲します。
